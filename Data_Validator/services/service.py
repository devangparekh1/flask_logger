...
class ValidateService:
    def push_to_es(self, data):
        try:
            ...
            return asyncio.run(send_data(data))
        except Exception as e:
            return False


async def send_data(data_to_send):
    today = date.today()
    try:
        ...
        asyncio.create_task(send_data_es(new_data_to_send))
        return True
    except Exception as e:
        return False


async def send_data_es(data_to_send):
    requests.post("http://127.0.0.1:4000/api/messages", json=data_to_send)
